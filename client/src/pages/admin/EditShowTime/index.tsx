...
import { GetShowtimeDetail, UpdateShowTime } from "@api/showtime/queries";
...
function EditShowTimeManager() {
  const { RangePicker } = DatePicker;
  const [formUpdateShowTime] = Form.useForm();
  const { id } = useParams();
  ...
  const { mutate: getShowTimeDetail } = GetShowtimeDetail();
  const { mutate: updateShowTime } = UpdateShowTime();
  ...
  useEffect(() => {
    getShowTimeDetail(Number(id), {
      onSuccess: (result) => {
        getFacilityByCinemaId(Number(result.data.data.hall.CinemaFacility.cinemaId), {
          onSuccess: (result) => {
            setFacilityList(result.data.data);
          },
        });

        getHall(Number(result.data.data.hall.CinemaFacility.id), {
          onSuccess: (result) => {
            setHallList(result.data.data);
          },
        });
        const startTime = dayjs(result.data.data.startTime).subtract(7, "hour");
        const endTime = dayjs(result.data.data.endTime).subtract(7, "hour");

        formUpdateShowTime.setFieldsValue({
          movieId: result.data.data.movieId,
          cinemaId: result.data.data.hall.CinemaFacility.cinemaId,
          facilityId: result.data.data.hall.CinemaFacility.id,
          hallId: result.data.data.hallId,
          showtime: [startTime, endTime],
        });
      },
    });
  }, [formUpdateShowTime, getFacilityByCinemaId, getHall, getShowTimeDetail, id]);
  ...
  const handleCreateShowTime = (value: { showtime: Date[]; hallId: number; movieId: number }) => {
    ...
    updateShowTime(
      {
        id: Number(id),
        data: { startTime, endTime, hallId: value.hallId, movieId: value.movieId, differenceInMinutes },
      },
      {
        onSuccess: () => {
          notification.success({ message: "Cập nhật xuất chiếu thành công!" });
          navigate(ROUTES.ADMIN.SHOWTIME);
        },
        onError: (e: any) => {
          formUpdateShowTime.setFields([
            {
              name: "showtime",
  ...
  return (
    <div className="p-10 pt-[70px]">
      ...
      <Form form={formUpdateShowTime} onFinish={handleCreateShowTime} layout="vertical">
        <Card className="bg-[#0f1d2f] box-shadow border-none text-[#8fbdff]">
          <Row gutter={[32, 32]}>
            ...
            <Col span={12}>
              <Form.Item name="cinemaId" label="Cinema:" rules={[{ required: true, message: "Không được để trống!" }]}>
                <Select
                  allowClear
                  onChange={(id) => {
                    formUpdateShowTime.setFieldsValue({ facilityId: undefined });
            ...
            <Col span={12}>
              <Form.Item label="Cơ sở:" name="facilityId" rules={[{ required: true, message: "Không được để trống!" }]}>
                <Select
                  allowClear
                  onChange={(id: number) => {
                    formUpdateShowTime.setFieldsValue({ hallId: undefined });
                    getHall(id, {
                      onSuccess: (result) => {
                        setHallList(result.data.data);
                      },
                  ...
                  disabled={!formUpdateShowTime.getFieldValue("cinemaId")}
                >
                  {renderFacility}
                </Select>
              </Form.Item>
            ...
            <Col span={12}>
              ...
              >
                <Select disabled={!formUpdateShowTime.getFieldValue("facilityId")}>{renderHall}</Select>
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
...
export default EditShowTimeManager;
